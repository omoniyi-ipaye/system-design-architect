#!/usr/bin/env python3
"""Render canonical System Design Architect JSON into a self-contained visual HTML report."""
...
def load_model(path: Path) -> dict:
    ...
    required = ("schema_version", "system", "elements", "flows")
    missing = [k for k in required if k not in data]
    if missing:
        raise SystemExit(f"Model missing required keys: {', '.join(missing)}")
    return data


def element_index(model: dict) -> dict:
    return {e["id"]: e for e in model.get("elements", [])}


def stage_items(items: list[dict], stage: str) -> list[dict]:
    return [x for x in items if x.get("stage") == stage]
...
def render_summary(model: dict) -> str:
    s = model["system"]
    outcomes = "".join(f"<li>{esc(x.get('text'))} {badge(x.get('evidence',''))}</li>" for x in s.get("desired_outcomes", []))
    return f"""
...
def render_stage_map(model: dict, stage: str) -> str:
    elems = stage_items(model.get("elements", []), stage)
    flows = stage_items(model.get("flows", []), stage)
    if not elems and not flows:
        return f'<section><h2>{STAGE_LABELS[stage]}</h2><p class="muted">No model elements for this stage.</p></section>'

    idx = {e["id"]: e for e in elems}
    # Preserve a readable deterministic order: actors/external first, then process/capability, state/queue/control last.
    order = {"actor":0,"external":0,"system":1,"service":1,"capability":2,"process":2,"decision":3,"state":4,"resource":4,"data_store":4,"queue":5,"control":5,"metric":6,"device":3}
    ...
    for i, e in enumerate(elems):
        ...
        meta = f"{esc(e.get('kind',''))} · {esc(e.get('evidence',''))}"
        nodes.append(f'<g class="node {esc(e.get("kind",""))}"><rect x="{x}" y="{y}" width="170" height="64" rx="12"/><text x="{x+85}" y="{y+26}" text-anchor="middle">{label}</text><text class="node-meta" x="{x+85}" y="{y+47}" text-anchor="middle">{meta}</text></g>')

    arrows = []
    for f in flows:
        if f.get("from") not in positions or f.get("to") not in positions:
            continue
        x1,y1 = positions[f["from"]]; x2,y2 = positions[f["to"]]
        x1 += 170; y1 += 32; y2 += 32
        if x2 < x1:
            x1 -= 170
        midx, midy = (x1+x2)/2, (y1+y2)/2 - 7
        arrows.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" marker-end="url(#arrow)"/><text class="edge-label" x="{midx}" y="{midy}" text-anchor="middle">{esc(f.get("label") or f.get("type"))}</text>')

    flow_legend = sorted({f.get("type") for f in flows if f.get("type")})
    legend = " ".join(badge(x, "flow") for x in flow_legend)
    return f"""
<section>
...
def render_risks(model: dict) -> str:
    ...
    if not risks:
        return ""
    rows = "".join(f"<tr><td>{badge(r.get('severity',''), 'sev-'+r.get('severity',''))}</td><td><strong>{esc(r.get('name'))}</strong><div class='muted'>{esc(r.get('cause'))}</div></td><td>{esc(r.get('control','—'))}</td><td>{esc(r.get('recovery','—'))}</td></tr>" for r in risks)
...
def render_capacity(model: dict) -> str:
    ...
    for c in caps:
        ...
        cards.append(f"<div class='card'><div class='eyebrow'>{esc(c.get('stage',''))}</div><h3>{esc(c.get('name'))}</h3><div class='metric-row'><span>Arrival</span><strong>{esc(arrival if arrival is not None else '—')} {esc(c.get('unit',''))}</strong></div><div class='metric-row'><span>Processing</span><strong>{esc(process if process is not None else '—')} {esc(c.get('unit',''))}</strong></div><div class='metric-row'><span>Status</span><strong>{esc(status)}</strong></div><p class='muted'>{esc(c.get('notes',''))}</p></div>")
...
def render_health(model: dict) -> str:
    ...
    if not signals and not recoveries:
        return ""
    signal_rows="".join(f"<tr><td><strong>{esc(s.get('name'))}</strong></td><td>{esc(s.get('desired_state'))}</td><td>{esc(s.get('threshold','—'))}</td><td>{esc(s.get('owner','—'))}</td></tr>" for s in signals)
    recovery_rows="".join(f"<tr><td>{badge(r.get('level',''))}</td><td>{esc(r.get('trigger'))}</td><td>{esc(r.get('action'))}</td><td>{esc(r.get('authority'))}</td><td>{esc(r.get('verification','—'))}</td></tr>" for r in recoveries)
    adapt=model.get("adaptation",{})
    allowed="".join(f"<li>{esc(x)}</li>" for x in adapt.get("allowed",[]))
    governed="".join(f"<li>{esc(x)}</li>" for x in adapt.get("requires_governed_redesign",[]))
    return f"""
...
<div class="adapt-grid"><div><h3>Allowed inside envelope {badge(adapt.get('level','—'))}</h3><ul>{allowed or '<li>None defined</li>'}</ul></div><div><h3>Requires governed redesign</h3><ul>{governed or '<li>None defined</li>'}</ul></div></div>
</section>"""
...
def render_transition(model: dict) -> str:
    ...
    for t in transitions:
        blocks.append(f"<div class='transition'><div>{badge(STAGE_LABELS.get(t.get('from'),t.get('from','')))}</div><div class='transition-arrow'>→</div><div class='transition-change'><strong>{esc(t.get('change'))}</strong><div class='muted'>Verify: {esc(t.get('verification','—'))}</div><div class='muted'>Rollback: {esc(t.get('rollback','—'))}</div></div><div class='transition-arrow'>→</div><div>{badge(STAGE_LABELS.get(t.get('to'),t.get('to','')))}</div></div>")
...
def render_fitness(model: dict) -> str:
    ...
    if not checks:
        return ""
    rows="".join(f"<tr><td>{esc(c.get('requirement'))}</td><td>{esc(c.get('mechanism'))}</td><td>{esc(c.get('verification'))}</td><td>{esc(c.get('pass_condition'))}</td><td>{esc(c.get('validation','—'))}</td></tr>" for c in checks)
...
def stylesheet() -> str:
    return """
:root{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#171717;background:#f7f7f5}*{box-sizing:border-box}body{margin:0}.page{max-width:1180px;margin:0 auto;padding:36px 22px 80px}.summary,section{background:white;border:1px solid #deded9;border-radius:18px;padding:24px;margin:0 0 18px}.eyebrow{text-transform:uppercase;letter-spacing:.09em;font-size:11px;font-weight:700;color:#6b6b65}.summary h1{font-size:38px;margin:6px 0 8px}.purpose{font-size:18px;color:#444;max-width:820px}.summary-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:20px}.summary-grid div,.card{border:1px solid #e6e6e1;border-radius:13px;padding:14px;background:#fbfbf9}.summary-grid span,.metric-row span{display:block;font-size:12px;color:#777}.summary-grid strong{display:block;margin-top:4px}.outcomes{margin-top:18px}.section-head{display:flex;justify-content:space-between;gap:16px;align-items:end}h2{font-size:25px;margin:3px 0 16px}h3{margin:4px 0 10px;font-size:16px}.badge{display:inline-block;border:1px solid #d7d7d1;border-radius:999px;padding:3px 8px;font-size:11px;background:#f4f4f1}.badge.flow{margin-left:4px}.sev-critical,.sev-high{font-weight:700}.viz-scroll{overflow-x:auto;border:1px solid #ecece8;border-radius:14px;background:#fcfcfa}.system-map{display:block;width:100%;min-width:720px;height:auto}.system-map line{stroke:#8b8b84;stroke-width:1.6}.system-map marker path{fill:#8b8b84}.node rect{fill:#fff;stroke:#a8a8a1;stroke-width:1.5}.node.control rect,.node.queue rect{stroke-dasharray:5 3}.node text{font-size:12px;font-weight:650;fill:#202020}.node .node-meta{font-size:9px;font-weight:500;fill:#73736d}.edge-label{font-size:9px;fill:#5f5f59}.table-wrap{overflow-x:auto}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;color:#6b6b65;font-size:11px;text-transform:uppercase;letter-spacing:.04em}th,td{padding:12px 10px;border-bottom:1px solid #ededE8;vertical-align:top}.muted{color:#74746e;font-size:12px;margin-top:4px}.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.metric-row{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid #eeeeea}.metric-row span{display:inline}.loop{display:flex;flex-wrap:wrap;align-items:center;gap:7px;margin:0 0 16px}.loop span{border:1px solid #dcdcd7;border-radius:10px;padding:8px 10px;background:#fafaf7;font-size:12px}.adapt-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:16px}.adapt-grid>div{border:1px solid #e6e6e1;border-radius:13px;padding:14px}.transition{display:grid;grid-template-columns:auto 35px 1fr 35px auto;align-items:center;gap:8px;border-top:1px solid #ecece8;padding:16px 0}.transition:first-of-type{border-top:0}.transition-arrow{text-align:center;font-size:20px;color:#777}@media(max-width:700px){.summary-grid{grid-template-columns:1fr 1fr}.adapt-grid{grid-template-columns:1fr}.transition{grid-template-columns:1fr}.transition-arrow{transform:rotate(90deg)}.summary h1{font-size:30px}}
"""


def render_html(model: dict) -> str:
    body = [render_summary(model)]
    for stage in STAGES:
        body.append(render_stage_map(model, stage))
    body.extend([render_transition(model), render_capacity(model), render_risks(model), render_fitness(model), render_health(model)])
    return "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>"+esc(model['system'].get('name'))+" — System Views</title><style>"+stylesheet()+"</style></head><body><main class='page'>"+"".join(x for x in body if x)+"</main></body></html>"
